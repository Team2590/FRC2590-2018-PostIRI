 ...
 * calculations, as well as writing the given TalonSRX.
 *
 * <p>This feedback controller runs in discrete time, so time deltas are not used in the integral
 * and derivative calculations. Therefore, the sample rate affects the controller's behavior for a
 * given set of PID constants.
 */
public class PIDTurnController extends SendableBase implements PIDInterface, Sendable{
  ...
  private double m_breakFreePoint = 0.0;
  private double m_lockupPoint = 0.0;
  private double m_turningOrNot = 0.0;
  ...
  protected NemesisMultiMC m_TalonSRX;
  ...
  private class PIDTurnTask extends TimerTask {
    private PIDTurnController m_controller;

    PIDTurnTask(PIDTurnController controller) {
   ...
   * @param output The TalonSRX object that is set to the output percentage
  ...
  public PIDTurnController(double Kp, double Ki, double Kd, double Kf, PIDSource source,
      NemesisMultiMC output, double period) {
    super(false);
    requireNonNull(source, "Null PIDSource was given");
    requireNonNull(output, "Null TalonSRX was given");
   ...
   */
  @SuppressWarnings("ParameterName")
  public PIDTurnController(double Kp, double Ki, double Kd, PIDSource source, NemesisMultiMC output,
                       double period) {
  ...
  /**
   * Set the control output at which the drivebase begins to turn
   */
  public void setBreakFreePoint(double v) {
    m_breakFreePoint = v;
  }
  
  /**
   * Set the control output at which the drivebase would stop turning if already moving
   */
  public void setLockupPoint(double v) {
    m_lockupPoint = v;
  }
  
  /**
   * Set the minimum turning rate at which the drivebase is defined to be moving and not stopped
   */
  public void setTurningDistinctionPoint(double v) {
    m_turningOrNot = v;
  }
  
  private boolean isTurning(double rate) {
    return rate > m_turningOrNot;
  }
  ...
  protected void calculate() {
    if (m_origSource == null || m_TalonSRX == null) {
      return;
    }
    ...
    if (enabled) {
      ...
      try {
        ...
        // Measure speed for when in position mode
        if (m_lastTime < 0) {speed = 0;}
        else {speed = Math.abs((input - m_prevInput) / (Timer.getFPGATimestamp() - m_lastTime));}
      ...
      if (pidSourceType.equals(PIDSourceType.kRate)) {
        ...
        // Calculate and apply friction correction (linearize the system)
        double frictionCorrection;
        
        if (isTurning(speed)) { 
          frictionCorrection = m_lockupPoint;
        } else {
          frictionCorrection = m_breakFreePoint;
        }
        
        frictionCorrection *= (result > 0) ? 1 : -1;
        
        if (result != 0) result += frictionCorrection;
      ...
      try {
        ...
        try {
          if (m_enabled) {
            ...
            m_TalonSRX.set(result);        
            SmartDashboard.putNumber("Power", result);
          }
        } finally {
